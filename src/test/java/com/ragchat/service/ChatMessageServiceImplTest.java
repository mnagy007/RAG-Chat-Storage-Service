...
class ChatMessageServiceImplTest {
    ...
    @Mock private ChatSessionRepo chatSessionRepo;
    ...
    @BeforeEach
    void setUp() {
        service = new ChatMessageServiceImpl(chatMessageRepo, chatSessionRepo);
    ...
    @Test
    void addMessage_whenSessionExistsAndNotDeleted_setsAssociationAndSaves() {
        ...
        verify(chatMessageRepo).save(msgCaptor.capture());
        assertEquals(sessionId, msgCaptor.getValue().getChatSession().getId());
    ...
    // ---------- history (uses private get() → findByIdAndDeletedAtIsNull) ----------

    @Test
    void history_whenSessionExists_returnsPagedMessages() {
        when(chatSessionRepo.findByIdAndDeletedAtIsNull(sessionId))
        ...
        verifyNoMoreInteractions(chatMessageRepo, chatSessionRepo);
    }

    @Test
    void history_whenSessionMissing_throwsNotFound_andDoesNotQueryMessages() {
        when(chatSessionRepo.findByIdAndDeletedAtIsNull(sessionId))
